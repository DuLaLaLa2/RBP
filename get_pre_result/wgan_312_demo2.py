...
class WGANGenerator(nn.Module):
    """只用标签作为条件，不使用任何特征统计"""
    def __init__(self, noise_dim: int = 64, label_dim: int = 32, feat_dim: int = 1408, hidden_dim: int = 512):
        super().__init__()
...
class WGANDiscriminator(nn.Module):
    def __init__(self, feat_dim: int = 1408, label_dim: int = 32, hidden_dim: int = 512):
        super().__init__()
        self.backbone = nn.Sequential(
...
def main():
    ...
    torch.save({
        ...
        'gat': gat.state_dict()
    }, "get_pre_result/data/wgan_312_demo2.pt")
    print("\n模型已保存")
