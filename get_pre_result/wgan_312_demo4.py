...
def pretrain_vgmm_gan_edge(
    ...
    return generator, edge_predictor, vgm_generator
...
def augment_dataset(
    ...
    for graph in train_graphs:
        ...
        # VGMM采样 + 噪声 + 标签
        vgm_s = torch.tensor(vgm_generator.sample(n_need), dtype=torch.float32).to(device)
        noise = torch.randn(n_need, noise_dim, device=device)
...
def main():
    ...
    # 预训练 (VGMM + WGAN + Edge)
    generator, edge_predictor, vgm_generator = pretrain_vgmm_gan_edge(
    ...
    torch.save({
        ...
        'gat': gat.state_dict()
    }, "get_pre_result/data/wgan_312_demo4.pt")
    print("\n模型已保存")
