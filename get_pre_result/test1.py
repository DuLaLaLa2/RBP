...
z_dim=64
    ...
    完整增强流程：
    1. WGAN 生成新节点特征
    2. Edge Builder + 置信模块自动补边
    基于：
    - WGAN-GP（生成节点特征）
    - Edge Builder + 置信度筛选（补边）
    target_ratio = 1.0 → 正负样本 1:1
    """
    ...
    for _ in range(need):
        ...
        fake_feat = G(z, c_proto).squeeze(0)  # [1408]
        new_feats.append(fake_feat)
    ...
    # 4. 把新节点“注册”进图（✅ 关键顺序）
    # =====================================================
    data.x = torch.cat([data.x, new_feats], dim=0)

    data.struct_feat = torch.cat(
        [data.struct_feat, new_feats[:, :data.struct_feat.size(1)]],
        dim=0
    )
    data.seq_feat = torch.cat(
        [data.seq_feat, new_feats[:, data.struct_feat.size(1):]],
        dim=0
    )

    data.y = torch.cat(
        [data.y,
         torch.ones(need, device=device, dtype=data.y.dtype)],
        dim=0
    )
...
if __name__ == "__main__":
    ...
    data_list = torch.load(
        "/root/myprojects/RBP_v2/get_graph_data/data/pyg_graph_datas_495_train.pt",
        weights_only=False
    )
    ...
    # =====================================================
    # Stage 3: 图增强（WGAN + Edge Builder + 置信模块）
    # =====================================================
    print("\n[Stage 3] Augmenting training graphs with confidence-aware edges...")
    ...
    torch.save(
        enhanced_all,
        "/root/myprojects/RBP_v2/get_pre_result/data/wgan_augment_result/pyg_495_enhance_datas.pt"
    )
