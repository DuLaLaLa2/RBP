...
def pretrain_gan_and_edge(
    ...
    """预训练WGAN + Edge Predictor，训练后冻结"""
    
    stat_conds = compute_stat_conds(train_graphs, device)
    label_embed_dim = 32
    cond_dim = label_embed_dim + 128
    ...
    for epoch in range(gan_epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            batch_label = torch.ones(len(idx), label_embed_dim, device=device)
...
def augment_dataset(
    ...
    for graph in train_graphs:
        ...
        # 生成节点
        batch_label = torch.ones(n_need, label_embed_dim, device=device)
...
def main():
    ...
    torch.save({
        ...
        'gat': gat.state_dict()
    }, "get_pre_result/data/wgan_312_demo1.pt")
    print("\n模型已保存")
