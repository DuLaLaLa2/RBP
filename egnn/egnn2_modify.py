...
from foclaloss import PracticalEasyFocusedLoss,HybridEasyFocusedLoss,LogSpaceEasyFocusedLoss
...
#     def __init__(self, alpha_t=0.25, gamma=2.0, reduction='mean'):
#         """
#         实现公式定义的Focal Loss：
#         FL = { -(α_t)*(1-P_t)^γ * log(P_t)  (y=1)
#              { -(1-α_t)*(P_t)^γ * log(1-P_t) (y=0)

#         FL = {-(1-α_t)*p_t^γ*log(1-p_t)}    (y=1)
#              {-α_t*(1-p_t)^γ*log(p_t)}  (y=0)
        
#         参数:
#             alpha_t: 类别权重系数（对应公式中的α_t）
#             gamma: 调制因子的指数（对应公式中的γ）
...
#         # logits: [batch_size, num_classes] (num_classes=2)
#         # targets: [batch_size] （0/1标签）
        
#         # 计算概率 P_t (正类概率)
#         probs = torch.softmax(logits, dim=1)  # [batch, 2]
#         p_t = probs[:, 1]  # 正类概率 P_t
        
#         # 分情况计算损失
#         # FL = { -(α_t)*(1-P_t)^γ * log(P_t)  (y=1)
#         #      { -(1-α_t)*(P_t)^γ * log(1-P_t) (y=0)
#         loss_0 = -(1 - self.alpha_t) * torch.pow(p_t, self.gamma) * torch.log(1-p_t + self.epsilon)
#         loss_1 = -self.alpha_t * torch.pow(1 - p_t, self.gamma) * torch.log(p_t + self.epsilon)
#         # FL = {-(1-α_t)*p_t^γ*log(1-p_t)}    (y=1)
#         #      {-α_t*(1-p_t)^γ*log(p_t)}    (y=0)
#         # loss_0 = -(self.alpha_t) * torch.pow(1-p_t, self.gamma) * torch.log(p_t + self.epsilon)
#         # loss_1 = -(1-self.alpha_t) * torch.pow(p_t, self.gamma) * torch.log(1-p_t + self.epsilon)
#         # 根据标签选择对应损失
#         loss = torch.where(targets == 0, loss_0, loss_1)
...
class FocalLoss(nn.Module):
    """
    关注易分样本的损失函数
    通过反转调制因子，使模型专注于已经能正确分类的样本
    
    Args:
        alpha (float): 正类样本的权重系数，取值范围 [0, 1]
                       - alpha > 0.5: 更关注正类
                       - alpha < 0.5: 更关注负类
                       - alpha = 0.5: 平衡关注
        gamma (float): 聚焦参数，控制对易分样本的关注程度
                       - gamma = 0: 退化为加权交叉熵
                       - gamma > 0: 越大的值越关注易分样本
                       - 推荐范围: [0.5, 3.0]
        epsilon (float): 数值稳定常数，防止 log(0)
    """
    def __init__(self, alpha_t=0.5, gamma=2.0, epsilon=1e-7, reduction='mean'):
        super().__init__()
        self.alpha = alpha_t      # 正类权重
    ...
    def forward(self, logits, targets):
        ...
        Args:
            ...
            targets: [batch_size] 真实标签，取值为 0 或 1
        ...
        p = probs[:, 1]  # 预测为正类的概率 [batch_size]
        
        # 数值稳定处理
        p = torch.clamp(p, self.epsilon, 1 - self.epsilon)
        
        # 关注易分样本的损失
        # y=1: -alpha * (p)^gamma * log(p)  # 易分正类 p大 -> 权重大
        # y=0: -(1-alpha) * (1-p)^gamma * log(1-p)  # 易分负类 1-p大 -> 权重大
        loss = torch.where(
            targets == 1,
...
class Trainer:
    def __init__(self, model, train_loader, val_loader, test_loader, device, lr=1e-3):
        ...
        # 替换为Focal Loss（可根据需求调整alpha_t和gamma）
        self.criterion = HybridEasyFocusedLoss(alpha=0.6, gamma=2.0)
